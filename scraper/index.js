...
const LIST_PATH = scraperConfig.apiListPath;
...
function locationTypeToWorkmode(locationType) {
  // BambooHR ATS: 1 = on-site, 2 = hybrid, 3 = remote
  ...
  return {
    jobs: result.map(job => {
      ...
      return {
        ...
        uid: String(job.id),
        workmode: locationTypeToWorkmode(job.locationType),
        location,
        tags: []
      };
    }),
...
async function scrapeAllListings(testOnlyOnePage = false) {
  ...
  const { jobs, total } = parseJobsPage(data);
  console.log(`Total jobs on BambooHR: ${total}`);
