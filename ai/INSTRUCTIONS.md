...
This scraper extracts job listings from TEC Agency Careers Romania API and imports them to peviitor.ro.

Target: https://tecss.bamboohr.com/careers
...
1. **Start with brand** - We know the brand ("TEC Agency")
...
8. **Scrape new jobs** - Extract jobs from TEC Agency Careers API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts TEC Agency Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from TEC Agency Careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape BambooHR careers JSON (jobs for Romania)
    │
    ▼
...
| `tests/unit/index.test.js` | Unit tests for parseJobsPage, mapToJobModel, transformJobsForSOLR |
...
- **BambooHR Careers**: `GET https://tecss.bamboohr.com/careers/list` — JSON with job openings
