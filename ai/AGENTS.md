...
- **Only the API parsing logic in `scraper/index.js`** (`fetchJobsPage`, `parseJobsPage`) is EPAM-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/tec-software-solutions-srl-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/tec-software-solutions-srl-nodejs-scraper --state open`
